...
def reduce_and_save(dataset_path, save_folder, dim, sample_size, mode, rand_state=597):
    ...
    # --- 2. Fit PCA ---
    pca_object = PCA(n_components=dim, random_state=rand_state)
    
    start_time = time.time()
    pca_object.fit(train_sampled)
    end_time = time.time()
    ...
    start_time = time.time()
    red_test = pca_object.transform(test)
    end_time = time.time()
    ...
    if mode == 'normal':
        ...
        red_train = pca_object.transform(train_full)
        del train_full
        ...
        with h5py.File(save_path, "a") as f_out:
            ...
            with h5py.File(dataset_path, "r") as f_in:
                ...
                for i in range(0, num_total_samples, BATCH_SIZE):
                    ...
                    red_batch = pca_object.transform(batch)
...
def main():
    ...
    parser.add_argument("sample_size", type=float, 
        help="4. Sample size (0.0 to 1.0) for fitting PCA.")
