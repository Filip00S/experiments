...
def reduce_and_save(dataset_path, save_folder, dim, sample_size, mode, param,rand_state=597):
    ...
    # --- 2. Fit UMAP ---
    umap_object = UMAP(n_components=dim, n_neighbors=param, random_state=rand_state, metric=metric, n_jobs=n_jobs)
    
    start_time = time.time()
    umap_object.fit(train_sampled)
    end_time = time.time()
    ...
    start_time = time.time()
    red_test = umap_object.transform(test)
    end_time = time.time()
    ...
    if mode == 'normal':
        ...
        red_train = umap_object.transform(train_full)
        del train_full
        ...
        with h5py.File(save_path, "a") as f_out:
            ...
            with h5py.File(dataset_path, "r") as f_in:
                ...
                for i in range(0, num_total_samples, BATCH_SIZE):
                    ...
                    red_batch = umap_object.transform(batch)
...
def main():
    ...
    parser.add_argument("sample_size", type=float, 
        help="4. Sample size (0.0 to 1.0) for fitting UMAP.")
